import{_ as s,c as a,a0 as p,o as e}from"./chunks/framework.P9qPzDnn.js";const u=JSON.parse('{"title":"调度中心启动流程","description":"","frontmatter":{},"headers":[],"relativePath":"xxl-job/xxl-job-admin-schedule.md","filePath":"xxl-job/xxl-job-admin-schedule.md"}'),l={name:"xxl-job/xxl-job-admin-schedule.md"};function t(i,n,o,r,c,g){return e(),a("div",null,n[0]||(n[0]=[p(`<h1 id="调度中心启动流程" tabindex="-1">调度中心启动流程 <a class="header-anchor" href="#调度中心启动流程" aria-label="Permalink to &quot;调度中心启动流程&quot;">​</a></h1><p>本章节我们一起进入xxl-job的调度器启动流程，讲解一下xxl-job的启动流程。</p><p>打开xxl-job-admin的com/xxl/job/admin/conf目录下的XxlJobAdminConfig，具体代码如下:</p><div class="language-XxlJobAdminConfig vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobAdminConfig</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Component</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到XxlJobAdminConfig继承了InitializingBean，在spring初始化过程中会调用afterPropertiesSet方法，调用xxlJobScheduler#init方法。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobScheduler  {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到xxlJobScheduler#init一共做了如下几件事：</p><ul><li>国际化初始化</li><li>触发器线程池创建（核心）</li><li>注册监控器启动（核心）</li><li>失败监控器启动（核心）</li><li>丢失监控器启动（核心）</li><li>日志任务启动</li><li>JobSchedule调度器启动（核心）</li></ul><h2 id="国际化资源初始化" tabindex="-1">国际化资源初始化 <a class="header-anchor" href="#国际化资源初始化" aria-label="Permalink to &quot;国际化资源初始化&quot;">​</a></h2><p>首先我们看看国际化资源初始化的initI18n方法，具体代码如下所示：</p><div class="language-XxlJobScheduler vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobScheduler</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobScheduler  {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>ExecutorBlockStrategyEnum是执行阻塞策略枚举，主要有单机串行、丢弃后续调度、覆盖之前调度三种策略，initI18n方法就是设置执行策略的title值。</p><p>可以看到是调用I18nUtil#getString获取的，具体代码如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class I18nUtil {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>I18nUtil.getString方法就是根据配置读取resources/il8n/目录下的其中一个文件，该目录下有message_en.properties、message_zh_CN.properties、message_zh_TC.properties三个文件，分别为英语、中文简体、中文繁体的属性文件。具体选取哪个文件是根据XxlJobAdminConfig.getAdminConfig().getI18n()获取的，我们看一下这个值是怎么取的。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Component</span></span>
<span class="line"><span>public class XxlJobAdminConfig implements InitializingBean, DisposableBean {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到就是从application.properties文件中获取到的xxl.job.i18n，如果没有，默认是zh_CN，而上章的默认配置也是zh_CN。</p><p>最后看一下PropertiesLoaderUtils#loadProperties的解析过程。</p><div class="language-PropertiesLoaderUtils vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">PropertiesLoaderUtils</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class PropertiesLoaderUtils {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="触发器线程池创建" tabindex="-1">触发器线程池创建 <a class="header-anchor" href="#触发器线程池创建" aria-label="Permalink to &quot;触发器线程池创建&quot;">​</a></h2><p>JobTriggerPoolHelper#toStart其实就是创建了两个线程池，一个快线程池，一个慢线程池。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobTriggerPoolHelper {</span></span>
<span class="line"><span>  </span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这两个配置就是在application.properties中定义的，默认配置如下：</p><div class="language-application.properties vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">application.properties</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>## xxl-job, triggerpool max size</span></span>
<span class="line"><span>xxl.job.triggerpool.fast.max=200</span></span>
<span class="line"><span>xxl.job.triggerpool.slow.max=100</span></span></code></pre></div><h2 id="注册中心监控线程启动" tabindex="-1">注册中心监控线程启动 <a class="header-anchor" href="#注册中心监控线程启动" aria-label="Permalink to &quot;注册中心监控线程启动&quot;">​</a></h2><p>JobRegistryHelper.getInstance().start()就是调度了JobRegistryHelper#start方法</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobRegistryHelper {</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>JobRegistryHelper#start创建了一个线程是跟一个守护线程，先看线程池。</p><div class="language-JobRegistryHelper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">JobRegistryHelper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobRegistryHelper {</span></span>
<span class="line"><span>  public void start() {</span></span>
...
<span class="line"><span>            });</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>看注释可以看出来是处理执行器的注册跟注册移除的。接下来查看守护线程。</p><div class="language-JobRegistryHelper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">JobRegistryHelper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobRegistryHelper {</span></span>
<span class="line"><span>  public void start() {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>注册监控器线程registryMonitorThread，调度任务注册线程池用来执行调度任务的注册，注册监控器线程用来监控执行器的机器是否下线。然后将registryMonitorThread设置为守护线程，最后启动registryMonitorThread线程，开始监控执行器的机器。</p><p>registryMonitorThread的run()方法一直执行，直到服务停止，主要做了两件事，第一将已经下线的执行器的记录从数据库中删除，第二将还在线的执行器机器记录重新设置执行器地址以及更新执行器的时间，然后更新数据库的记录。怎么判定执行器已经下线了？如果数据库中的update_time字段小于当前时间减去死亡期限，那么说明已经执行器在死亡期限没有进行更新时间，就判定已经下线了。执行器在启动的时候，会启动一个执行器线程不断的执行注册任务，执行器任务会更新update_time字段。</p><h2 id="失败监控器启动" tabindex="-1">失败监控器启动 <a class="header-anchor" href="#失败监控器启动" aria-label="Permalink to &quot;失败监控器启动&quot;">​</a></h2><p>接下来看JobFailMonitorHelper.getInstance().start()方法。</p><div class="language-JobFailMonitorHelper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">JobFailMonitorHelper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobFailMonitorHelper {</span></span>
<span class="line"><span>	public void start(){</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>run方法一直运行，直到线程停止。run方法的首先从数据库中获取失败的调度任务日志列表，每次最多一千条。遍历失败的调度任务日志列表，首先将失败的调度任务日志进行锁定，暂停给告警邮件发送告警信息。如果调度任务的失败重试次数大于0，触发任务执行，更新任务日志信息。当邮件不为空时，触发告警信息，最后将锁定的日志状态更新为告警状态。</p><h2 id="丢失监控器启动" tabindex="-1">丢失监控器启动 <a class="header-anchor" href="#丢失监控器启动" aria-label="Permalink to &quot;丢失监控器启动&quot;">​</a></h2><p>接下来查看JobCompleteHelper.getInstance().start()方法。这个方法声明了一个线程池和一个守护线程，我们先来看线程池的配置。</p><div class="language-JobCompleteHelper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">JobCompleteHelper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobCompleteHelper {</span></span>
<span class="line"><span>  public void start(){</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这个线程池是用来处理日志回调服务的，即“执行器”在接收到任务执行请求后，执行任务，在执行结束之后会将执行结果回调通知“调度中心”, 执行器调用/api/callback就会调用该线程池, 具体看接口处理流程，这里不细说。接下来看守护线程。</p><div class="language-JobCompleteHelper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">JobCompleteHelper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobCompleteHelper {</span></span>
<span class="line"><span>	public void start() {</span></span>
<span class="line"><span>		// for monitor</span></span>
<span class="line"><span>		monitorThread = new Thread(new Runnable() {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>		monitorThread.start();</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>monitorThread就是每分钟扫描一下调度记录，如果调度记录运行中状态超时且执行器不在线，则标记为失败状态。</p><h2 id="日志任务启动" tabindex="-1">日志任务启动 <a class="header-anchor" href="#日志任务启动" aria-label="Permalink to &quot;日志任务启动&quot;">​</a></h2><p>JobLogReportHelper.getInstance().start()是一个清除日志服务，对过期的日志任务进行清除。</p><div class="language-JobLogReportHelper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">JobLogReportHelper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobLogReportHelper {</span></span>
<span class="line"><span>    public void start(){</span></span>
...
<span class="line"><span>    }  </span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="jobschedule调度器启动" tabindex="-1">JobSchedule调度器启动 <a class="header-anchor" href="#jobschedule调度器启动" aria-label="Permalink to &quot;JobSchedule调度器启动&quot;">​</a></h2><p>JobScheduleHelper.getInstance().start()，这个方法比较核心，下一篇文章讲解。</p>`,50)]))}const b=s(l,[["render",t]]);export{u as __pageData,b as default};
