import{_ as n,c as a,a0 as e,o as p}from"./chunks/framework.P9qPzDnn.js";const g=JSON.parse('{"title":"路由选择策略","description":"","frontmatter":{},"headers":[],"relativePath":"xxl-job/route.md","filePath":"xxl-job/route.md"}'),t={name:"xxl-job/route.md"};function l(i,s,r,c,o,u){return p(),a("div",null,s[0]||(s[0]=[e(`<h1 id="路由选择策略" tabindex="-1">路由选择策略 <a class="header-anchor" href="#路由选择策略" aria-label="Permalink to &quot;路由选择策略&quot;">​</a></h1><p>上一篇我们在介绍定时任务执行流程的时候，在processTrigger章节有提到了路由策略如果不是分片广播策略，那么就会根据定时任务设置的路由选择策略选取执行器的地址。</p><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这章内容我们就来讲解一下XXL-JOB的路由选取策略，保证在多个执行器地址中选择其中一个地址来执行定时任务。</p><h2 id="executorroutestrategyenum" tabindex="-1">ExecutorRouteStrategyEnum <a class="header-anchor" href="#executorroutestrategyenum" aria-label="Permalink to &quot;ExecutorRouteStrategyEnum&quot;">​</a></h2><p>想要了解XXL-JOB的路由选取策略，ExecutorRouteStrategyEnum这个类是必须了解的内容，代码如下所示:</p><div class="language-ExecutorRouteStrategyEnum vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorRouteStrategyEnum</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public enum ExecutorRouteStrategyEnum {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到除了SHARDING_BROADCAST分片广播策略之外，还有9种路由策略，下面做一下解释：</p><ul><li>FIRST（第一个）：固定选择第一个机器；</li><li>LAST（最后一个）：固定选择最后一个机器；</li><li>ROUND（轮询）： 轮训策略；</li><li>RANDOM（随机）：随机选择在线的机器；</li><li>CONSISTENT_HASH（一致性HASH）：每个任务按照Hash算法固定选择某一台机器，且所有任务均匀散列在不同机器上。</li><li>LEAST_FREQUENTLY_USED（最不经常使用）：使用频率最低的机器优先被选举；</li><li>LEAST_RECENTLY_USED（最近最久未使用）：最久未使用的机器优先被选举；</li><li>FAILOVER（故障转移）：按照顺序依次进行心跳检测，第一个心跳检测成功的机器选定为目标执行器并发起调度；</li><li>BUSYOVER（忙碌转移）：按照顺序依次进行空闲检测，第一个空闲检测成功的机器选定为目标执行器并发起调度；</li><li>SHARDING_BROADCAST(分片广播)：广播触发对应集群中所有机器执行一次任务，同时系统自动传递分片参数；可根据分片参数开发分片任务；</li></ul><p>下面让我们逐个分析上面这就中路由策略。</p><h2 id="executorroutefirst" tabindex="-1">ExecutorRouteFirst <a class="header-anchor" href="#executorroutefirst" aria-label="Permalink to &quot;ExecutorRouteFirst&quot;">​</a></h2><p>FIRST策略比较简单，直接看代码，就是选择地址列表中的第一个地址。</p><div class="language-ExecutorRouteFirst vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorRouteFirst</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteFirst extends ExecutorRouter {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="executorroutelast" tabindex="-1">ExecutorRouteLast <a class="header-anchor" href="#executorroutelast" aria-label="Permalink to &quot;ExecutorRouteLast&quot;">​</a></h2><p>LAST策略也比较简单，直接看代码，就是选择地址列表中的最后一个地址。</p><div class="language-ExecutorRouteLast vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorRouteLast</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteLast extends ExecutorRouter {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @Override</span></span>
<span class="line"><span>  public ReturnT&lt;String&gt; route(TriggerParam triggerParam, List&lt;String&gt; addressList) {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="executorrouteround" tabindex="-1">ExecutorRouteRound <a class="header-anchor" href="#executorrouteround" aria-label="Permalink to &quot;ExecutorRouteRound&quot;">​</a></h2><p>ROUND策略就是先随机选择一个Index，然后通过Index对地址列表的元素进行取余，获取执行器的地址，后面累加1取余, 代码如下。</p><div class="language-ExecutorRouteRound vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorRouteRound</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteRound extends ExecutorRouter {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="executorrouterandom" tabindex="-1">ExecutorRouteRandom <a class="header-anchor" href="#executorrouterandom" aria-label="Permalink to &quot;ExecutorRouteRandom&quot;">​</a></h2><p>RANDOM策略也比较简单，生成一个0到地址列表元素个数之间的数值，然后将该数值作为下标拿到执行器地址即可。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteRandom extends ExecutorRouter {</span></span>
...
<span class="line"><span>    return new ReturnT&lt;String&gt;(address);</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="executorrouteconsistenthash" tabindex="-1">ExecutorRouteConsistentHash <a class="header-anchor" href="#executorrouteconsistenthash" aria-label="Permalink to &quot;ExecutorRouteConsistentHash&quot;">​</a></h2><p>CONSISTENT_HASH（一致性HASH）：每个任务按照Hash算法固定选择某一台机器，且所有任务均匀散列在不同机器上。</p><div class="language-ExecutorRouteConsistentHash vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorRouteConsistentHash</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteConsistentHash extends ExecutorRouter {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><p>CONSISTENT_HASH（一致性哈希算法）对地址列表中的每个地址做100次哈希运算，根据hash值从虚拟节点与执行器地址对应关系获取对应的执行器地址返回。</p><h2 id="executorroutelfu" tabindex="-1">ExecutorRouteLFU <a class="header-anchor" href="#executorroutelfu" aria-label="Permalink to &quot;ExecutorRouteLFU&quot;">​</a></h2><p>LEAST_FREQUENTLY_USED（最不经常使用）：使用频率最低的机器优先被选举。</p><div class="language-ExecutorRouteLFU vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorRouteLFU</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteLFU extends ExecutorRouter {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    return new ReturnT&lt;String&gt;(address);</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这里也用到了ConcurrentHashMap存储，key是任务id，val是一个linkHashMap，按照访问顺序进行排序的，所以每次选取时，直接拿entryKey的元素即可。</p><h2 id="executorroutefailover" tabindex="-1">ExecutorRouteFailover <a class="header-anchor" href="#executorroutefailover" aria-label="Permalink to &quot;ExecutorRouteFailover&quot;">​</a></h2><p>FAILOVER（故障转移）：按照顺序依次进行心跳检测，第一个心跳检测成功的机器选定为目标执行器并发起调度；</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteFailover extends ExecutorRouter {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @Override</span></span>
<span class="line"><span>  public ReturnT&lt;String&gt; route(TriggerParam triggerParam, List&lt;String&gt; addressList) {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>ExecutorRouteFailover是失败转移路由，route方法遍历执行器地址，然后发送心跳给执行器服务，如果心跳正常，则成功返回该执行器地址，否则返回失败码。</p><h2 id="executorroutebusyover" tabindex="-1">ExecutorRouteBusyover <a class="header-anchor" href="#executorroutebusyover" aria-label="Permalink to &quot;ExecutorRouteBusyover&quot;">​</a></h2><p>BUSYOVER（忙碌转移）：按照顺序依次进行空闲检测，第一个空闲检测成功的机器选定为目标执行器并发起调度。</p><div class="language-ExecutorRouteBusyover vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorRouteBusyover</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorRouteBusyover extends ExecutorRouter {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @Override</span></span>
<span class="line"><span>  public ReturnT&lt;String&gt; route(TriggerParam triggerParam, List&lt;String&gt; addressList) {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>ExecutorRouteBusyover是忙碌转移路由器，route方法首先遍历执行器地址列表，然后对执行器地址进行空闲检测，当任务线程没有在执行定时任务时，将返回空闲检测成功，将该执行器地址返回。</p>`,43)]))}const R=n(t,[["render",l]]);export{g as __pageData,R as default};
