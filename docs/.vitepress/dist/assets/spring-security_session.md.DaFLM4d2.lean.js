import{_ as n,c as a,a0 as e,o as t}from"./chunks/framework.P9qPzDnn.js";const p="/assets/getAuthentication.oXiSqZfb.png",i="/assets/SecurityContextHolder-SecurityContext-Authentication.Crh-PGAV.png",l="/assets/strategy.C3ytTukw.png",o="/assets/Servlet3SecurityContextHolderAwareRequestWrapper.V27FFCar.png",r="/assets/SecurityContextRepository.B5wOAIyv.png",c="/assets/SessionAuthenticationStrategy.CV5g9HhY.png",u="/assets/session-share.CLUTnIcY.png",v=JSON.parse('{"title":"用户会话管理","description":"","frontmatter":{},"headers":[],"relativePath":"spring-security/session.md","filePath":"spring-security/session.md"}'),d={name:"spring-security/session.md"};function g(h,s,S,y,C,x){return t(),a("div",null,s[0]||(s[0]=[e(`<h1 id="用户会话管理" tabindex="-1">用户会话管理 <a class="header-anchor" href="#用户会话管理" aria-label="Permalink to &quot;用户会话管理&quot;">​</a></h1><p>用户登录成功后，是怎么保存用户的会话信息的呢？在代码中我们怎么样获取用户的认证信息呢？还有用户怎么样可以做到多设备登录等。这些是本章讲解的内容。</p><p>跟之前一样，我们还是基于Spring Security的配置文件开始讲起。</p><div class="language-DefaultSecurityConfig vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">DefaultSecurityConfig</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="用户登录数据获取" tabindex="-1">用户登录数据获取 <a class="header-anchor" href="#用户登录数据获取" aria-label="Permalink to &quot;用户登录数据获取&quot;">​</a></h2><p>登录成功之后，在后续的业务逻辑中，开发者有可能需要获取登录成功的用户对象，如果不使用任何框架，可以将用户数据保存到HttpSession中，然后从HttpSession获取。</p><p>在Spring Security中，用户信息还是保存在HttpSession中，但是为了方便使用，Spring Security对用户信息进行了封装。有两种方式可以对用户的登录数据进行获取。</p><ul><li>从SecurityContextHolder中获取</li><li>从当前请求对象中获取</li></ul><p>上一篇认证流程有提到，最终用户认证的信息是保存在Authentication中的, 用户登录成功后获取到用户登录数据也是这个，这里重新放一遍:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface Authentication extends Principal, Serializable {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>而基于表单登录的Authentication实现类是UsernamePasswordAuthenticationToken</p><h3 id="securitycontextholder" tabindex="-1">SecurityContextHolder <a class="header-anchor" href="#securitycontextholder" aria-label="Permalink to &quot;SecurityContextHolder&quot;">​</a></h3><p>先演示一下登录成功后怎么从SecurityContextHolder中获取用户的认证信息。新建一个Controller，名字叫做UserController，代码如下：</p><div class="language-UserController vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">UserController</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@RestController</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>启动项目，登录成功后，访问/user接口, 就会给我们返回用户的认证信息，如下图所示:</p><img src="`+p+'" width="400" alt="getAuthentication"><p>可以看到SecurityContextHolder.getContext()是一个静态方法，返回SecurityContext, 由此我们可以推断出SecurityContextHolder，SecurityContext，Authentication三者的关系</p><img src="'+i+`" width="350" alt="SecurityContextHolder-SecurityContext-Authentication三者的关系"><p>我们还是从SecurityContextHolder这个类开始看起吧，先看getContext，具体代码如下:</p><div class="language-SecurityContextHolder vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextHolder</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SecurityContextHolder {</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到是调用了strategy.getContext返回的，那么strategy是在哪里赋值的呢？继续看代码</p><div class="language-SecurityContextHolder vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextHolder</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SecurityContextHolder {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到主要的strategy有三种：</p><ul><li>MODE_THREADLOCAL: 将SecurityContext存放在ThreadLocal中，可以实现单线程共享，默认策略</li><li>MODE_INHERITABLETHREADLOCAL: 将SecurityContext存放在Inheritable ThreadLocal中，可以实现子线程共享，看需要配置</li><li>MODE_GLOBAL: 将SecurityContext存放到一个变量中，全局共享，基本用不到，在多用户环境下会有冲突</li></ul><p>可以看到默认的strategy是通过System.getProperty加载的，我们可以通过配置系统变量来修改默认的存储策略。</p><p><img src="`+l+`" alt="strategy"></p><p>打开IDEA的项目配置，然后在虚拟机选项里面修改strategy即可，比如设置为MODE_INHERITABLETHREADLOCAL</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>-Dspring.security.strategy=MODE_INHERITABLETHREADLOCAL</span></span></code></pre></div><h3 id="securitycontextholderstrategy" tabindex="-1">SecurityContextHolderStrategy <a class="header-anchor" href="#securitycontextholderstrategy" aria-label="Permalink to &quot;SecurityContextHolderStrategy&quot;">​</a></h3><p>可以看到strategy的类的SecurityContextHolderStrategy，我们看一下这个接口的定义:</p><div class="language-SecurityContextHolderStrategy vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextHolderStrategy</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface SecurityContextHolderStrategy {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>可以看到是存在InheritableThreadLocal，学过InheritableThreadLocal的都知道，这里的值会跟着子线程的创建传递给子线程的InheritableThreadLocal。所以是子线程共享的。</p><p>最后我们看一下GlobalSecurityContextHolderStrategy，代码如下</p><div class="language-GlobalSecurityContextHolderStrategy vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">GlobalSecurityContextHolderStrategy</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>final class GlobalSecurityContextHolderStrategy implements SecurityContextHolderStrategy {</span></span>
<span class="line"><span>	private static SecurityContext contextHolder;</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到就是保存在静态变量里面的，这个策略基本不用，相当于某一时刻，所有用户共享一个认证信息了。</p><p>好了，讨论完SecurityContextHolder，我们看一下SecurityContext是在哪个过滤器里面赋值的。</p><h3 id="securitycontext" tabindex="-1">SecurityContext <a class="header-anchor" href="#securitycontext" aria-label="Permalink to &quot;SecurityContext&quot;">​</a></h3><p>查看SecurityContext的接口声明，可以看到只有获取Authentication跟设置Authentication的方法</p><div class="language-SecurityContext vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContext</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface SecurityContext extends Serializable {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>这个接口默认使用的实现类是SecurityContextImpl。</p><h3 id="securitycontextholderfilter" tabindex="-1">SecurityContextHolderFilter <a class="header-anchor" href="#securitycontextholderfilter" aria-label="Permalink to &quot;SecurityContextHolderFilter&quot;">​</a></h3><p>加下来我们看看Authentication是怎么放入SecurityContextHolder中的。首先我们来看HttpSecurity Bean的声明函数</p><div class="language-HttpSecurityConfiguration vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">HttpSecurityConfiguration</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration(proxyBeanMethods = false)</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>上面只调重点的说，可以看到http引入了securityContext这了函数，这个函数往HttpSecurity中加入了SecurityContextConfigurer。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class HttpSecurity extends AbstractConfiguredSecurityBuilder&lt;DefaultSecurityFilterChain, HttpSecurity&gt;</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>按照惯例，我们看看SecurityContextConfigurer的configure方法</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class SecurityContextConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt;</span></span>
...
<span class="line"><span>		}</span></span>
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>SecurityContextRepository本节后面讲Session的时候会用到。可以看到this.requireExplicitSave默认是true，往过滤器中加入了SecurityContextHolderFilter这个过滤器。</p><div class="warning custom-block"><p class="custom-block-title">WARNING</p><p>SecurityContextPersistenceFilter在Spring Security6中被标注为@Deprecated</p></div><p>接下来我们看一下SecurityContextHolderFilter这个过滤器的实现,看doFilter方法。</p><div class="language-SecurityContextHolderFilter vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextHolderFilter</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SecurityContextHolderFilter extends GenericFilterBean {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>好了，到这，大家应该明白SecurityContextHolder.getContext().getAuthentication()是怎么获取的了。注意返回的是deferredContext，这个有懒加载的意思。我们就拿ThreadLocalSecurityContextHolderStrategy来看。</p><div class="language-ThreadLocalSecurityContextHolderStrategy vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ThreadLocalSecurityContextHolderStrategy</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>final class ThreadLocalSecurityContextHolderStrategy implements SecurityContextHolderStrategy {</span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到在getContext的时候才会把SecurityContext放到contextHolder中。</p><h3 id="从请求对象中获取" tabindex="-1">从请求对象中获取 <a class="header-anchor" href="#从请求对象中获取" aria-label="Permalink to &quot;从请求对象中获取&quot;">​</a></h3><p>除了SecurityContextHolder，还有另一种方式可以获取用户的Authentication-从请求中获取。在UserController中新建一个接口</p><div class="language-UserController vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">UserController</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@RestController</span></span>
<span class="line"><span>public class UserController {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>登录成功后，访问/authentication接口一样可以获取Authentication。</p><p>了解Servlet的都知道，Controller方法的参数都是当前请求HttpServletRequest带来的。而HttpServletRequest遵循Servlet规范，要我们看看这个接口跟用户相关的方法</p><div class="language-HttpServletRequest vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">HttpServletRequest</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface HttpServletRequest extends ServletRequest {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>默认的实现RequestFacade。不过为了实现直接获取Authentication，Spring Security利用装饰器模式，重新封装了一个类Servlet3SecurityContextHolderAwareRequestWrapper</p><img src="`+o+`" width="300" alt="Servlet3SecurityContextHolderAwareRequestWrapper"><p>让我们看看Servlet3SecurityContextHolderAwareRequestWrapper跟用户登录信息相关的实现，代码如下</p><div class="language-SecurityContextHolderAwareRequestWrapper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextHolderAwareRequestWrapper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SecurityContextHolderAwareRequestWrapper extends HttpServletRequestWrapper {</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到，这些有关用户登录的数据都可以在Controller中直接获取。在Spring Security的过滤器中，有一个SecurityContextHolderAwareRequestFilter的过滤器，会对HttpServletRequest进行包装，查看代码。</p><div class="language-SecurityContextHolderAwareRequestFilter vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextHolderAwareRequestFilter</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SecurityContextHolderAwareRequestFilter extends GenericFilterBean {</span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到在调用doFilter的时候，传下去了一个requestFactory创建的HttpServletRequest，而requestFactory是通过createServlet3Factory创建的，查看代码。</p><div class="language-SecurityContextHolderAwareRequestFilter vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextHolderAwareRequestFilter</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SecurityContextHolderAwareRequestFilter extends GenericFilterBean {</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>跟进HttpServlet3RequestFactory的create方法。代码如下:</p><div class="language-HttpServlet3RequestFactory vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">HttpServlet3RequestFactory</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>final class HttpServlet3RequestFactory implements HttpServletRequestFactory {</span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>好了，Servlet3SecurityContextHolderAwareRequestWrapper的创建到此为止。接下来我们看一下SecurityContextHolderAwareRequestFilter是怎么加入过滤器中的。</p><p>按照惯例我们查看HttpSecurity Bean的声明函数。</p><div class="language-HttpSecurityConfiguration vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">HttpSecurityConfiguration</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration(proxyBeanMethods = false)</span></span>
<span class="line"><span>class HttpSecurityConfiguration {</span></span>
<span class="line"><span>  @Bean(HTTPSECURITY_BEAN_NAME)</span></span>
<span class="line"><span>	@Scope(&quot;prototype&quot;)</span></span>
<span class="line"><span>	HttpSecurity httpSecurity() throws Exception {</span></span>
...
<span class="line"><span>    ...</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>点击servletApi进去查看，往HttpSecurity注入了ServletApiConfigurer，很好，我们来看ServletApiConfigurer的configure函数。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class ServletApiConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt;</span></span>
...
<span class="line"><span>    ...</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>好了，分析到这里为止，至于Controller中怎么可以直接使用Authentication这个类的，这里是Spring的内容，后面会新出一个课程讲解。</p><h3 id="authentication保存" tabindex="-1">Authentication保存 <a class="header-anchor" href="#authentication保存" aria-label="Permalink to &quot;Authentication保存&quot;">​</a></h3><p>那么Authentication是什么时候写入到SecurityContext中的呢，既然我们知道了是用户认证成功之后才会有Authentication的，那我们直接看认证成功的地方。前面提到过，用户的认证有一个抽象类Filter，在里面做认证以及认证成功处理。之前有个地方没有细讲，这里直接看代码。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractAuthenticationProcessingFilter extends GenericFilterBean</span></span>
...
<span class="line"><span>    ...</span></span>
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>Spring Security中使用securityContextRepository来保存SecurityContext, 然后前面在介绍SecurityContextHolderFilter的doFilter实现时，有从securityContextRepository获取用户信息的调用，如下所示:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SecurityContextHolderFilter extends GenericFilterBean {</span></span>
<span class="line"><span>  private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)</span></span>
<span class="line"><span>      throws ServletException, IOException {</span></span>
<span class="line"><span>    ...</span></span>
<span class="line"><span>    // 可以看到SecurityContext是从this.securityContextRepository中获取的,然后放入到securityContextHolderStrategy中</span></span>
...
<span class="line"><span>    ...</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>我们先来看一下securityContextRepository的声明，代码如下所示:</p><div class="language-SecurityContextRepository vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextRepository</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface SecurityContextRepository {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p><img src="`+r+`" alt="SecurityContextRepository"></p><p>可以看到实现类有5个，下面介绍一下常用的几个实现类：</p><ul><li>HttpSessionSecurityContextRepository: 将SecurityContext保存在HttpSession中</li><li>RequestAttributeSecurityContextRepository: 将SecurityContext保存在RequestAttribute，这个感觉在微服务架构中很有用 = DelegatingSecurityContextRepository: 复合Repository，可以嵌套SecurityContextRepository</li></ul><p>而SecurityContextRepository的默认类就是DelegatingSecurityContextRepository。前面说到HttpSecurity会引入以下这个SecurityContextConfigurer，看里面的configure方法。</p><div class="language-SecurityContextConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SecurityContextConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class SecurityContextConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt;</span></span>
<span class="line"><span>		extends AbstractHttpConfigurer&lt;SecurityContextConfigurer&lt;H&gt;, H&gt; {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  public void configure(H http) {</span></span>
<span class="line"><span>    SecurityContextRepository securityContextRepository = getSecurityContextRepository();</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>如果使用了SessionManagement，SecurityContextRepository是在SessionManagementConfigurer的init方法中声明的。SessionManagement在下一节用户会话并发管理会讲到。</p><div class="language-SessionManagementConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SessionManagementConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class SessionManagementConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt;</span></span>
...
<span class="line"><span>    ...</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>我们看一下DelegatingSecurityContextRepository的loadDeferredContext和saveContext方法。</p><div class="language-DelegatingSecurityContextRepository vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">DelegatingSecurityContextRepository</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class DelegatingSecurityContextRepository implements SecurityContextRepository {</span></span>
<span class="line"><span>  </span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到saveContext是遍历RequestAttributeSecurityContextRepository和HttpSessionSecurityContextRepository的saveContext方法 loadDeferredContext是遍历调用RequestAttributeSecurityContextRepository和HttpSessionSecurityContextRepository的loadDeferredContext方法。</p><p>我们这里只看HttpSessionSecurityContextRepository的实现吧。代码如下所示:</p><div class="language-HttpSessionSecurityContextRepository vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">HttpSessionSecurityContextRepository</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class HttpSessionSecurityContextRepository implements SecurityContextRepository {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="用户会话并发管理" tabindex="-1">用户会话并发管理 <a class="header-anchor" href="#用户会话并发管理" aria-label="Permalink to &quot;用户会话并发管理&quot;">​</a></h2><p>前面提到用户的认证信息是保存在HttpSession中的，浏览器通过在每次发送请求的时候发送一个SessionId，服务器端根据这个SessionId来判断用户的身份。当浏览器关闭后，服务端的Session不会销毁，需要在调用服务端接口销毁，或者等待Session过期。这些是Servlet的规范，不细讲。</p><p>会话并发管理是在当前系统中，同一个用户可以创建多少个会话，如果一个设备对应一个会话，那么也可以理解为同一个用户可以在多少台设备上进行登录，默认情况下，用户可以登录的设备是没有限制。仔细想想也知道，认证信息是保存在HttpSession中的，而HttpSession是根据SessionId区分，每个请求的SessionId不一样，只要存储同一个用户的认证信息就好了。</p><p>不过如果开发者想要限制用户的登录设备数的话，也可以设置，并且很简单。我们直接基于本节开头的DefaultSecurityConfig配置改造。具体代码如下：</p><div class="language-DefaultSecurityConfig vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">DefaultSecurityConfig</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration</span></span>
<span class="line"><span>public class DefaultSecurityConfig {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @Bean</span></span>
<span class="line"><span>  SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {</span></span>
...
<span class="line"><span>    return http.build();</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>启动项目，然后用chrome登录用户，访问/user接口，可以看到返回了一下信息</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>{&quot;authorities&quot;:[],&quot;details&quot;:{&quot;remoteAddress&quot;:&quot;0:0:0:0:0:0:0:1&quot;,&quot;sessionId&quot;:&quot;AA19F62AD0B34F3220A680A48F599263&quot;},&quot;authenticated&quot;:true,&quot;principal&quot;:{&quot;password&quot;:null,&quot;username&quot;:&quot;user&quot;,&quot;authorities&quot;:[],&quot;accountNonExpired&quot;:true,&quot;accountNonLocked&quot;:true,&quot;credentialsNonExpired&quot;:true,&quot;enabled&quot;:true},&quot;credentials&quot;:null,&quot;name&quot;:&quot;user&quot;}</span></span></code></pre></div><p>我们用另外一个浏览器，比如safari，登录用户，访问/user接口，也可以看到上面的信息。</p><p>好了，这个时候回到chrome，重新访问/user接口，可以发现会话过期了。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>This session has been expired (possibly due to multiple concurrent logins being attempted as the same user).</span></span></code></pre></div><p>这是一种后来者把之前的用户踢出去的做法，当然，也可以设置成后面的用户不可以登录，配置改成下面的就可以了。</p><div class="language-DefaultSecurityConfig vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">DefaultSecurityConfig</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration</span></span>
<span class="line"><span>public class DefaultSecurityConfig {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @Bean</span></span>
<span class="line"><span>  SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {</span></span>
...
<span class="line"><span>		return HttpSecurity.this;</span></span>
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>查看SessionManagementConfigurer的init方法和configure方法，先看init方法</p><div class="language-SessionManagementConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SessionManagementConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class SessionManagementConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt;</span></span>
<span class="line"><span>		extends AbstractHttpConfigurer&lt;SessionManagementConfigurer&lt;H&gt;, H&gt; {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    if (securityContextRepository == null) {</span></span>
<span class="line"><span>      // 如果是stateless，就啥都不敢</span></span>
<span class="line"><span>      if (stateless) {</span></span>
<span class="line"><span>        http.setSharedObject(SecurityContextRepository.class, new RequestAttributeSecurityContextRepository());</span></span>
<span class="line"><span>        this.sessionManagementSecurityContextRepository = new NullSecurityContextRepository();</span></span>
<span class="line"><span>      }</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这里简单介绍一下HttpSecurtiy的创建时机吧，在Spring Security中，一共有四种</p><ul><li>ALWAYS: 如果HttpSession不存在，则创建</li><li>NEVER: 从不创建HttpSession，但是如果HttpSession已经存在了，则会使用它</li><li>IF_REQUIRED: 当有需要时，会创建HttpSession，默认是这个</li><li>STATELESS: 从不创建HttpSession，也不使用HttpSession。这适合无状态认证方式</li></ul><p>再看configure方法。代码如下所示:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class SessionManagementConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt;</span></span>
<span class="line"><span>		extends AbstractHttpConfigurer&lt;SessionManagementConfigurer&lt;H&gt;, H&gt; {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @Override</span></span>
<span class="line"><span>  public void configure(H http) {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>接下来一步一步将会话并发管理控制。</p><h3 id="sessionauthenticationstrategy" tabindex="-1">SessionAuthenticationStrategy <a class="header-anchor" href="#sessionauthenticationstrategy" aria-label="Permalink to &quot;SessionAuthenticationStrategy&quot;">​</a></h3><p>首先要做会话管理，肯定是记录每个登录用户的会话信息的，在Spring Security中，这个会话信息的类是SessionInformation。具体如下所示:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SessionInformation implements Serializable {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>  ...</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="sessionregistry" tabindex="-1">SessionRegistry <a class="header-anchor" href="#sessionregistry" aria-label="Permalink to &quot;SessionRegistry&quot;">​</a></h3><p>SessionRegistry是一个接口，用来维护SessionInformation实例，该接口只有一个实现类SessionRegistryImpl，所以我们直接看SessionRegistryImpl。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SessionRegistryImpl implements SessionRegistry, ApplicationListener&lt;AbstractSessionEvent&gt; {</span></span>
...
<span class="line"><span>    });</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这个类主要定义了两个属性，介绍一下用处</p><ul><li>principals: 保存当前用户与SessionId之间的关系，也就是当前用户的会话id</li><li>sessionIds: 保存SessionId与SessionInformation之间的关系</li></ul><p>当用户登录成功后，会执行会话保存操作，传入当前请求的SessionId和当前登录主体的principal对象，如果SessionId已经存在，则先移除后，在保存新的。</p><h3 id="sessionauthenticationstrategy-1" tabindex="-1">SessionAuthenticationStrategy <a class="header-anchor" href="#sessionauthenticationstrategy-1" aria-label="Permalink to &quot;SessionAuthenticationStrategy&quot;">​</a></h3><p>SessionAuthenticationStrategy是一个接口，当用户登录成功后，会调用这个接口的onAuthentication方法。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface SessionAuthenticationStrategy {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>RegisterSessionAuthenticationStrategy的代码如下所示。</p><div class="language-RegisterSessionAuthenticationStrategy vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">RegisterSessionAuthenticationStrategy</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class RegisterSessionAuthenticationStrategy implements SessionAuthenticationStrategy {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到很简单，直接往sessionRegistry注册一个新的session。</p><p>CompositeSessionAuthenticationStrategy的代码如下所示。</p><div class="language-CompositeSessionAuthenticationStrategy vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">CompositeSessionAuthenticationStrategy</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class CompositeSessionAuthenticationStrategy implements SessionAuthenticationStrategy {</span></span>
...
<span class="line"><span>		}</span></span>
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到，直接遍历它维护的SessionAuthenticationStrategy的onAuthentication方法。</p><p>我们回到AbstractAuthenticationProcessingFilter认证成功的地方，可以看到在认证成功后会调用所属的SessionAuthenticationStrategy的onAuthentication方法。</p><div class="language-AbstractAuthenticationProcessingFilter vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AbstractAuthenticationProcessingFilter</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractAuthenticationProcessingFilter extends GenericFilterBean</span></span>
<span class="line"><span>		implements ApplicationEventPublisherAware, MessageSourceAware {</span></span>
<span class="line"><span>	</span></span>
<span class="line"><span>  private SessionAuthenticationStrategy sessionStrategy = new NullAuthenticatedSessionStrategy();</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>    successfulAuthentication(request, response, chain, authenticationResult);</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>sessionStrategy默认是个NullAuthenticatedSessionStrategy什么都不干，我们看看是在哪里赋值的。如果看了表单认证那个章节，就大概可以猜出是在AbstractAuthenticationFilterConfigurer里面配置的。</p><div class="language-AbstractAuthenticationFilterConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AbstractAuthenticationFilterConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractAuthenticationFilterConfigurer&lt;B extends HttpSecurityBuilder&lt;B&gt;, T extends AbstractAuthenticationFilterConfigurer&lt;B, T, F&gt;, F extends AbstractAuthenticationProcessingFilter&gt;</span></span>
<span class="line"><span>		extends AbstractHttpConfigurer&lt;T, B&gt; {</span></span>
<span class="line"><span>	@Override</span></span>
<span class="line"><span>	public void configure(B http) throws Exception {</span></span>
...
<span class="line"><span>		}</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>SessionAuthenticationStrategy.class是在哪里呢，由于本节是session会话管理，直接看SessionManagementConfigurer。</p><div class="language-SessionManagementConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">SessionManagementConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class SessionManagementConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt;</span></span>
<span class="line"><span>		extends AbstractHttpConfigurer&lt;SessionManagementConfigurer&lt;H&gt;, H&gt; {</span></span>
<span class="line"><span>	@Override</span></span>
<span class="line"><span>	public void init(H http) {</span></span>
<span class="line"><span>    http.setSharedObject(SessionAuthenticationStrategy.class, getSessionAuthenticationStrategy(http));</span></span>
...
<span class="line"><span>		return this.sessionAuthenticationStrategy;</span></span>
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><h3 id="sessionmanagementfilter" tabindex="-1">SessionManagementFilter <a class="header-anchor" href="#sessionmanagementfilter" aria-label="Permalink to &quot;SessionManagementFilter&quot;">​</a></h3><p>和会话并发管理相关的过滤器有两个，sessionManagementFilter和ConcurrentSessionFilter。我们分别看一下吧。</p><div class="language-sessionManagementFilter vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">sessionManagementFilter</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class SessionManagementFilter extends GenericFilterBean {</span></span>
<span class="line"><span>  </span></span>
<span class="line"><span>  private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)</span></span>
<span class="line"><span>			throws IOException, ServletException {</span></span>
...
<span class="line"><span>		chain.doFilter(request, response);</span></span>
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>我们再看看ConcurrentSessionFilter这个过滤器，代码如下所示：</p><div class="language-ConcurrentSessionFilter vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ConcurrentSessionFilter</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ConcurrentSessionFilter extends GenericFilterBean {</span></span>
<span class="line"><span>  </span></span>
<span class="line"><span>  private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)</span></span>
<span class="line"><span>      throws IOException, ServletException {</span></span>
<span class="line"><span>    HttpSession session = request.getSession(false);</span></span>
...
<span class="line"><span>    chain.doFilter(request, response);</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到，会从sessionRegistry获取会话信息，然后查看当前会话是否已经过期，如果已经过期了，则调用doLogout登出，并且调用会话过期方法。否则刷新最后一次访问时间。</p><h2 id="会话固定攻击" tabindex="-1">会话固定攻击 <a class="header-anchor" href="#会话固定攻击" aria-label="Permalink to &quot;会话固定攻击&quot;">​</a></h2><p>会话固定攻击指的是攻击者通过访问应用程序来创建会话，然后诱导用户以相同的会话ID登录（通常指的是将会话ID放在请求链接中，然后诱导用户去点击），进而获得用户的权限。</p><ul><li>攻击者登录网站，服务端给攻击者分配了一个SessionId</li><li>攻击者利用自己的SessionId构造一个网站链接，并将该链接发给受害者</li><li>受害者用和攻击者相同的SessionId进行登录，这时攻击者的SessionId在服务端就存储受害者的信息了</li><li>攻击者利用SessionId获取受害者的信息，冒充受害者操作</li></ul><p>在这个过程中，如果网站支持URL重写，那么攻击会更简单。用户如果在浏览器中禁用了cookie，那么sessionid也就用不了，有的服务器就支持把sessionid放到请求地址中，类似下面</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>http://www.javaboy.org;jsessionid=xxxx</span></span></code></pre></div><p>攻击者构造一个这样的地址很简单</p><p>Spring Security从三方面防范会话固定攻击：</p><ul><li>Spring Security自带Http防火墙，如果sessionid放在地址栏中，这个请求会被拦截</li><li>在Http相应的Set-Cookie字段中设置httpOnly，避免通过XSS攻击获取Cookie中的会话信息</li><li>既然会话固定攻击是由于SessionId不变导致的，所以可以在用户登录成功后，改变SessionId就可以了。</li></ul><p>关于第三种方案，Spring Security支持一种会话策略，名字叫做ChangeSessionIdAuthenticationStrategy, 配置起来也很简单。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration</span></span>
<span class="line"><span>public class DefaultSecurityConfig {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @Bean</span></span>
<span class="line"><span>  SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {</span></span>
...
<span class="line"><span>    return http.build();</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>sessionFixation有几种策略，不同的策略对应不同的SessionAuthenticationStrategy</p><ul><li>changeSessionId： 默认策略，只改变SessionId</li><li>none: 不做任何处理</li><li>migrateSession: 登录成功后，创建一个新的HttpSession对象，并将旧的对象拷贝过来</li><li>newSession: 登录成功后，创建一个新的HttpSession对象，拷贝一些属性</li></ul><p>我们大致看一下ChangeSessionIdAuthenticationStrategy吧，源码实现很简单。</p><div class="language-ChangeSessionIdAuthenticationStrategy vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ChangeSessionIdAuthenticationStrategy</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class ChangeSessionIdAuthenticationStrategy extends AbstractSessionFixationProtectionStrategy {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>	@Override</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>由于ChangeSessionIdAuthenticationStrategy继承了AbstractSessionFixationProtectionStrategy，所以看AbstractSessionFixationProtectionStrategy的onAuthentication。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractSessionFixationProtectionStrategy</span></span>
...
<span class="line"><span>&lt;/dependency&gt;</span></span></code></pre></div><p>修改application.yml, 添加redis的配置：</p><div class="language-application.yml vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">application.yml</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>spring:</span></span>
...
<span class="line"><span>      port: 6379</span></span></code></pre></div><p>新增一个SecurityConfig，代码如下：</p><div class="language-DefaultSecurityConfig vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">DefaultSecurityConfig</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>然后复制一个UserController，代码如下:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@RestController</span></span>
<span class="line"><span>public class UserController {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>  @GetMapping(&quot;/user&quot;)</span></span>
<span class="line"><span>  public UsernamePasswordAuthenticationToken userInfo() {</span></span>
<span class="line"><span>    return (UsernamePasswordAuthenticationToken) SecurityContextHolder.getContext().getAuthentication();</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>启动项目，访问/user接口，跳转登录页面，登录成功后，访问/user正常。这时我们直接看redis数据库，会发现多了一些KEY，类似下面的，则运行成功。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>spring:session:expirations:1735938180000</span></span>
...
<span class="line"><span>spring:session:sessions:expires:08cd7505-a8de-4473-9623-0c2ef7fe7a3b</span></span></code></pre></div><p>具体代码，点击<a href="https://github.com/shengduiliang/spring-security-demo/tree/main/spring-security-redis-session" target="_blank" rel="noreferrer">此处</a></p>`,201)]))}const m=n(d,[["render",g]]);export{v as __pageData,m as default};
