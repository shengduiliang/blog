import{_ as s,c as a,a0 as p,o as e}from"./chunks/framework.P9qPzDnn.js";const d=JSON.parse('{"title":"定时任务执行流程","description":"","frontmatter":{},"headers":[],"relativePath":"xxl-job/trigger.md","filePath":"xxl-job/trigger.md"}'),t={name:"xxl-job/trigger.md"};function l(i,n,r,o,g,c){return e(),a("div",null,n[0]||(n[0]=[p(`<h1 id="定时任务执行流程" tabindex="-1">定时任务执行流程 <a class="header-anchor" href="#定时任务执行流程" aria-label="Permalink to &quot;定时任务执行流程&quot;">​</a></h1><p>上一章我们分析了xxl-job是通过时间轮算法来调度定时任务的, 并且最后我们知道调用定时任务执行是通过下面这个方法。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>JobTriggerPoolHelper.trigger()</span></span></code></pre></div><p>这章分析一下定时任务是怎么被触发执行的。</p><h2 id="trigger" tabindex="-1">trigger <a class="header-anchor" href="#trigger" aria-label="Permalink to &quot;trigger&quot;">​</a></h2><p>我们看一下JobTriggerPoolHelper的trigger方法，代码如下：</p><div class="language-JobTriggerPoolHelper vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">JobTriggerPoolHelper</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class JobTriggerPoolHelper {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>根据任务ID从数据库中获取要执行的任务，然后判断路由策略，如果是分片广播，遍历地址列表，触发所有的机器，否则只触发一台机器。分片广播是要触发所有的机器并行处理任务。</p><h2 id="processtrigger" tabindex="-1">processTrigger <a class="header-anchor" href="#processtrigger" aria-label="Permalink to &quot;processTrigger&quot;">​</a></h2><p>接下来我们看一下processTrigger方法，这个方法的代码比较长，让我们一步一步分析。</p><ul><li>获取阻塞策略，路由策略以及分片广播参数等</li></ul><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>保存任务日志</li></ul><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
<span class="line"><span>  private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>初始化触发参数</li></ul><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
<span class="line"><span>  private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>初始化执行器的地址：如果路由策略是分片广播，执行地址就为第index的地址，否则从通过路由策略获取执行地址。(核心)</li></ul><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
<span class="line"><span>  private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>触发远程执行器，即触发远程的定时任务(核心)</li></ul><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
<span class="line"><span>  private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><ul><li>设置触发信息并保存触发日志</li></ul><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
<span class="line"><span>  private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="runexecutor" tabindex="-1">runExecutor <a class="header-anchor" href="#runexecutor" aria-label="Permalink to &quot;runExecutor&quot;">​</a></h2><p>主要看一下runExecutor方法，这个是实际的调度执行方法，代码如下</p><div class="language-XxlJobTrigger vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobTrigger</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobTrigger {</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>runExecutor通过XxlJobScheduler#getExecutorBiz获取到ExecutorBiz，然后调用executorBiz.run方法，等待返回结果，然后将结果返回。</p><p>ExecutorBiz有两个实现类，ExecutorBizClient和ExecutorBizImpl，xxl-job-admin使用的是ExecutorBizClient，查看ExecutorBizClient#run方法</p><div class="language-ExecutorBizClient vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">ExecutorBizClient</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ExecutorBizClient implements ExecutorBiz {</span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>ExecutorBizClient的run方法比较简单，就是通过XxlJobRemotingUtil发送一个请求，触发执行器里面的机器执行，然后把结果返回来。</p><h2 id="postbody" tabindex="-1">postBody <a class="header-anchor" href="#postbody" aria-label="Permalink to &quot;postBody&quot;">​</a></h2><p>最后我们还是看一下XxlJobRemotingUtil#postBody方法吧， 具体代码如下：</p><div class="language-XxlJobRemotingUtil vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">XxlJobRemotingUtil</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class XxlJobRemotingUtil {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到是构造了一个HttpURLConnection发起了一个HTTP请求，然后利用GsonTool解析返回结果。</p>`,37)]))}const b=s(t,[["render",l]]);export{d as __pageData,b as default};
