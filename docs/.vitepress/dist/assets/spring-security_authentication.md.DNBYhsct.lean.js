import{_ as s,c as a,a0 as e,o as t}from"./chunks/framework.P9qPzDnn.js";const p="/assets/FormLoginConfigurerExtends.Dlwc-2-Y.png",i="/assets/UsernamePasswordAuthenticationFilter.1FTCGBpV.png",l="/assets/AuthenticationManager.C__AW3xx.png",r="/assets/Authentication.B6jtmhgZ.png",c="/assets/DaoAuthenticationProvider.Elwhzr_C.png",o="/assets/abstractauthenticationprocessingfilter.BgCovxf1.png",u="/assets/AuthenticationProviders.B9K8N4WQ.png",h="/assets/providermanagers-parent.EjbwfLnF.png",d="/assets/UserDetailsService.BOFkPIbG.png",g="/assets/GlobalAuthenticationConfigurerAdapter.Cbffe-nT.png",A="/assets/AuthenticationManagerBuilder.D0tjyD5g.png",y=JSON.parse('{"title":"认证流程分析","description":"","frontmatter":{},"headers":[],"relativePath":"spring-security/authentication.md","filePath":"spring-security/authentication.md"}'),b={name:"spring-security/authentication.md"};function v(f,n,m,C,P,S){return t(),a("div",null,n[0]||(n[0]=[e(`<h1 id="认证流程分析" tabindex="-1">认证流程分析 <a class="header-anchor" href="#认证流程分析" aria-label="Permalink to &quot;认证流程分析&quot;">​</a></h1><p>本文将基于分析表单登录认证流程对Spring Security的认证流程梳理一遍。跟之前一样，我们还是基于Spring Security的配置文件开始讲起。</p><div class="language-DefaultSecurityConfig vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">DefaultSecurityConfig</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="formloginconfigurer" tabindex="-1">FormLoginConfigurer <a class="header-anchor" href="#formloginconfigurer" aria-label="Permalink to &quot;FormLoginConfigurer&quot;">​</a></h2><p><img src="`+p+`" alt="FormLoginConfigurer继承图"></p><p>前面有提到过，http.httpBasic其实是往HttpSecurit加入了FormLoginConfigurer这个configurer，而在HttpSecurity的build过程中，密切相关的是configurer#init跟configurer#configure方法，当然还有构造方法，所以我们先分析这三个接口。</p><p>首先看FormLoginConfigurer的初始化方法</p><div class="language-FormLoginConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">FormLoginConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public final class FormLoginConfigurer&lt;H extends HttpSecurityBuilder&lt;H&gt;&gt; extends</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>查看父类AbstractAuthenticationFilterConfigurer的构造函数，代码如下</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractAuthenticationFilterConfigurer&lt;B extends HttpSecurityBuilder&lt;B&gt;, T extends AbstractAuthenticationFilterConfigurer&lt;B, T, F&gt;, F extends AbstractAuthenticationProcessingFilter&gt;</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>注意UsernamePasswordAuthenticationFilter是重点，这个就是在过滤链中负责用户登录的过滤器。</p><p>接下来我们看AbstractAuthenticationFilterConfigurer的init函数，看一下做了什么事情。</p><div class="language-AbstractAuthenticationFilterConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AbstractAuthenticationFilterConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractAuthenticationFilterConfigurer&lt;B extends HttpSecurityBuilder&lt;B&gt;, T extends AbstractAuthenticationFilterConfigurer&lt;B, T, F&gt;, F extends AbstractAuthenticationProcessingFilter&gt;</span></span>
<span class="line"><span>		extends AbstractHttpConfigurer&lt;T, B&gt; {</span></span>
<span class="line"><span>	</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到init方法主要是对loginPage，loginProcessUrl，failureUrl，logoutSuccessUrl等进行配置和放行。</p><p>我们再看看里面的configure方法，看看做了什么。</p><div class="language-AbstractAuthenticationFilterConfigurer vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AbstractAuthenticationFilterConfigurer</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractAuthenticationFilterConfigurer&lt;B extends HttpSecurityBuilder&lt;B&gt;, T extends AbstractAuthenticationFilterConfigurer&lt;B, T, F&gt;, F extends AbstractAuthenticationProcessingFilter&gt;</span></span>
<span class="line"><span>		extends AbstractHttpConfigurer&lt;T, B&gt; {</span></span>
<span class="line"><span>  </span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到configure方法主要是对UsernamePasswordAuthenticationFilter做了一些设置和初始化。这里有必要说一下AbstractAuthenticationFilterConfigurer，因为后面如果我们要自定义自己的认证逻辑，很大可能会继承这个类来做。</p><h2 id="usernamepasswordauthenticationfilter" tabindex="-1">UsernamePasswordAuthenticationFilter <a class="header-anchor" href="#usernamepasswordauthenticationfilter" aria-label="Permalink to &quot;UsernamePasswordAuthenticationFilter&quot;">​</a></h2><p>上面我们知道了FormLoginConfigurer主要是往过SecurityFilterChain加入UsernamePasswordAuthenticationFilter这个过滤器。</p><p><img src="`+i+`" alt="UsernamePasswordAuthenticationFilter"></p><p>由于SecurityFilterChain在遍历每个Filter的时候，都会调用每个Filter的doFilter方法。所以我们直接看doFilter，这个方法是在UsernamePasswordAuthenticationFilter的父类AbstractAuthenticationProcessingFilter进行声明的。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public abstract class AbstractAuthenticationProcessingFilter extends GenericFilterBean</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到，主要是调用子类的attemptAuthentication，即UsernamePasswordAuthenticationFilter，接下来看这个方法:</p><div class="language-UsernamePasswordAuthenticationFilter vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">UsernamePasswordAuthenticationFilter</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class UsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {</span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到现在主要调用this.getAuthenticationManager().authenticate(authRequest)来进行验证。</p><p>接下来就讲解Spring Security关于认证相关的类和请求了。</p><h2 id="authenticationmanager" tabindex="-1">AuthenticationManager <a class="header-anchor" href="#authenticationmanager" aria-label="Permalink to &quot;AuthenticationManager&quot;">​</a></h2><p>AuthenticationManager是一个认证管理器，它定义了Spring Security要怎么处理认证操作。可以看到AuthenticationManager在认证成功之后会返回一个Authentication对象。这个对象会保存在SecurityContextHoler中（后面章节会讲到，现在可以理解为, 认证成功之后， 要找一个地方保存用户信息，用户再次请求，需要找到该认证信息, 确认用户身份）。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@FunctionalInterface</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>在HttpSecurity继承FormLoginConfigurer，可以看里面的configure方法，是在AbstractAuthenticationFilterConfigurer中实现的。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>\`\`\` AbstractAuthenticationFilterConfigurer</span></span>
...
<span class="line"><span>    this.authFilter.setAuthenticationManager(http.getSharedObject(AuthenticationManager.class));</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h2 id="providermanager" tabindex="-1">ProviderManager <a class="header-anchor" href="#providermanager" aria-label="Permalink to &quot;ProviderManager&quot;">​</a></h2><p>可以看到UsernamePasswordAuthenticationFilter是调用AuthenticationManager的authenticate方法进行注册的。所以我们直接看ProviderManager#authenticate。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class ProviderManager implements AuthenticationManager, MessageSourceAware, InitializingBean {</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到是遍历providers，找到合适的provider进行校验。</p><h2 id="authentication" tabindex="-1">Authentication <a class="header-anchor" href="#authentication" aria-label="Permalink to &quot;Authentication&quot;">​</a></h2><p>可以看到查找provider是根据Authentication来查找的，解析一下这个类。Authentication类就是用来存放用户的认证信息。具体代码如下所示：</p><div class="language-Authentication vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">Authentication</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface Authentication extends Principal, Serializable {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>可以看到只要获取到了Authentication对象，就可以获取到登录用户的详细信息。不同的认证方法对应不同的认证信息，比如用户/密码登录，OAuth2登录，RememberMe登录等，认证信息是不一样的，所以针对不同的认证方式会有不同的Authentication实现，既然Authentication都不一样了，具体认证的AuthenticationProvider也要不一样，这就很好理解了。</p><p><img src="`+r+`" alt="Authentication"></p><p>SpringSecurity默认提供了15个Authentication的实现。我们大概讲一下几个比较重要的：</p><ul><li>RememberMeAuthenticationToken: 如果使用RememberMe的方式，用户的信息保存在RememberMeAuthenticationToken中</li><li>UsernamePasswordAuthenticationToken: 如果使用FormLogin的方式，用户的信息保存在UsernamePasswordAuthenticationToken中</li></ul><p>这里每一种实现，都会有对应的AuthenticationProvider做验证，比如UsernamePasswordAuthenticationToken使用的是DaoAuthenticationProvider</p><h2 id="authenticationprovider" tabindex="-1">AuthenticationProvider <a class="header-anchor" href="#authenticationprovider" aria-label="Permalink to &quot;AuthenticationProvider&quot;">​</a></h2><p>可以看到AuthenticationProvider接口就只有两个方法。</p><div class="language-AuthenticationProvider vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AuthenticationProvider</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface AuthenticationProvider {</span></span>
...
<span class="line"><span>		return result;</span></span>
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>接下来我们查看DaoAuthenticationProvider对应方法的实现</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class DaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {</span></span>
<span class="line"><span>  @Override</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>好了，表单登录的基本流程到这里结束了。</p><h2 id="认证流程总结" tabindex="-1">认证流程总结 <a class="header-anchor" href="#认证流程总结" aria-label="Permalink to &quot;认证流程总结&quot;">​</a></h2><p>接下来对Spring Security的认证过程做一个总结</p><p><img src="`+o+'" alt="abstractauthenticationprocessingfilter"></p><p>当请求经过Spring Security的SecurityFilterChain后，认证过滤器AbstractAuthenticationProcessingFilter会生成一个Authentication对象，将这个对象交给AuthenticationManager进行认证，认证成功则会走认证成功逻辑，失败则走失败认证逻辑</p><p><img src="'+u+'" alt="AuthenticationProviders"></p><p>默认的AuthenticationManager实现是ProviderManager， ProviderManager会遍历自己管理的AuthenticationProvider列表，对Authentication对象进行认证，认证成功返回成功的Authentication对象</p><p><img src="'+h+`" alt="providermanagers-parent.png"></p><p>如果ProviderManager认证失败，会将认证转交给自己的父类AuthenticationManager进行认证。</p><h2 id="补充" tabindex="-1">补充 <a class="header-anchor" href="#补充" aria-label="Permalink to &quot;补充&quot;">​</a></h2><p>在DaoAuthenticationProvider的retrieveUser方法中，用到了UserDetailsService这个类，在调用this.getUserDetailsService().loadUserByUsername(username)的时候;</p><p>很容易看出这是获取用户信息的，我们看一下UserDetailsService的定义：</p><div class="language-UserDetailsService vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">UserDetailsService</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public interface UserDetailsService {</span></span>
<span class="line"><span>	UserDetails loadUserByUsername(String username) throws UsernameNotFoundException;</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>查看UserDetailsService有哪些实现类，可以看到默认有6种，主要是下面两种：</p><ul><li>InMemoryUserDetailsManager: 把用户信息放在内存上进行查询</li><li>JdbcUserDetailsManager: 使用JDBC从数据库中获取用户信息</li></ul><p><img src="`+d+`" alt="UserDetailsService"></p><p>看了第一节配置用户信息那部分，很容易就知道DaoAuthenticationProvider的UserDetailsService默认就是自动配置的那个InMemoryUserDetailsManager。</p><div class="language-UserDetailsServiceAutoConfiguration vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">UserDetailsServiceAutoConfiguration</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Bean</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>那么这个UserDetailsService是怎么设置到DaoAuthenticationProvider的呢？</p><p>还有一个问题, 我们知道ProviderManager是在HttpSecurity的beforeConfigure中初始化，DaoAuthenticationProvider又是什么时候注册到ProviderManager里面的呢？</p><p>之前分析过，Spring Boot继承Spring Security的时候会自动注入@EnableWebSecurity这个注解，我们重新看一下这个注解</p><div class="language-EnableWebSecurity vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">EnableWebSecurity</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Import({ WebSecurityConfiguration.class, SpringWebMvcImportSelector.class, OAuth2ImportSelector.class,</span></span>
...
<span class="line"><span>  ...</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到引入@EnableGlobalAuthentication这个注解，查看这个注解</p><div class="language-EnableGlobalAuthentication vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">EnableGlobalAuthentication</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Import(AuthenticationConfiguration.class)</span></span>
<span class="line"><span>public @interface EnableGlobalAuthentication {</span></span>
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到引入了AuthenticationConfiguration.class这个类，看名字就知道是认证配置的类，那我们就详细来了解一下这个类。</p><div class="language-AuthenticationConfiguration vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AuthenticationConfiguration</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration(proxyBeanMethods = false)</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到声明了好几个Bean，下面说明一下这几个Bean。</p><ul><li>AuthenticationManagerBuilder: AuthenticationManager构建器，返回一个AuthenticationManagerBuilder，用来构造AuthenticationManager，默认实现是DefaultPasswordEncoderAuthenticationManagerBuilder。</li><li>GlobalAuthenticationConfigurerAdapter: 返回一个EnableGlobalAuthenticationAutowiredConfigurer，可以认为什么都不干，不重要</li><li>InitializeUserDetailsBeanManagerConfigurer：看这个Bean的名字就知道跟UserDetailsService有关了，后面分析</li><li>InitializeAuthenticationProviderBeanManagerConfigurer: 看这个Bean的名字就知道跟AuthenticationProvider有关了，后面分析</li></ul><p>InitializeUserDetailsBeanManagerConfigurer跟InitializeAuthenticationProviderBeanManagerConfigurer都继承了GlobalAuthenticationConfigurerAdapter这个类</p><img src="`+g+`" width="300" alt="GlobalAuthenticationConfigurerAdapter"><p>看继承图，是不是很熟悉，肯定有init跟configure方法。刚好注入这两个类的是setGlobalAuthenticationConfigurers，把它们赋给globalAuthConfigurers。</p><p>接下来我们回看声明HttpSecurity Bean的代码。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Configuration(proxyBeanMethods = false)</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>authenticationManager()实际调用的是上面声明的AuthenticationConfiguration的getAuthenticationManager方法，我们查看这个方法。</p><div class="language-AuthenticationConfiguration vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AuthenticationConfiguration</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class AuthenticationConfiguration {</span></span>
...
<span class="line"><span>    return this.authenticationManager;</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>查看AuthenticationManagerBuilder的继承图，是不是很熟悉。</p><img src="`+A+`" width="300" alt="AuthenticationManagerBuilder"><p>容易看出，authBuilder.build()会遍历globalAuthConfigurers，执行init方法跟configure方法。</p><h3 id="initializeuserdetailsbeanmanagerconfigurer" tabindex="-1">InitializeUserDetailsBeanManagerConfigurer <a class="header-anchor" href="#initializeuserdetailsbeanmanagerconfigurer" aria-label="Permalink to &quot;InitializeUserDetailsBeanManagerConfigurer&quot;">​</a></h3><p>先看InitializeUserDetailsBeanManagerConfigurer，具体代码如下</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Order(InitializeUserDetailsBeanManagerConfigurer.DEFAULT_ORDER)</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看出来，这里是初始化了DaoAuthenticationProvider，并把UserDetailsService设置到DaoAuthenticationProvider。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Order(InitializeAuthenticationProviderBeanManagerConfigurer.DEFAULT_ORDER)</span></span>
...
<span class="line"><span>		}</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>这里就是从spring中获取AuthenticationProvider的Bean，注册到AuthenticationManagerBuilder。</p><p>最后让我们看一下DefaultPasswordEncoderAuthenticationManagerBuilder的performBuild方法，该类继承了AuthenticationManagerBuilder</p><div class="language-AuthenticationManagerBuilder vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">AuthenticationManagerBuilder</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>public class AuthenticationManagerBuilder</span></span>
...
<span class="line"><span>	}</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>可以看到就是返回了一个providerManager，这个就是HttpSecurity里面的providerManager的父类providerManager。</p><p>而HttpSecurity里面的providerManager之前提过了是在HttpSecurity的beforeConfigure构建的。</p>`,102)]))}const M=s(b,[["render",v]]);export{y as __pageData,M as default};
